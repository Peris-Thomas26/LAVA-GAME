...
Lava Game
Done by LYIMO Peris Thomas .SID:21145542
"""
...
def add_lava():
    ...
    if game_being_played:
        ...
        for x in range(quantity_of_lava_cells_to_add_each_time):
...
def handle_if_gold(row, col):
    ...
    if game_being_played:
        ...
        #target_gold_coins==4
        if map[row][col]==2: #contains gold (the value 2)
            map[row][col]=0 #put a zero in that cell, because that gold is now taken by the player
            player_gold_coins=player_gold_coins+1#increase the number stored in player_gold_coins by 1
        #printhow many target_gold_coins gold coins the player now has
            print('The player now has',player_gold_coins,'coins')
